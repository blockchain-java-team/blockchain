 ...
 * them to the miner that mines that block.
 * We are america btw.
 */
public class BlockchainData {
     ...
     * <br>
     * call the {@link Transaction#isVerified(Signature)} and
     * {@link Block#isVerified(Signature)} methods.
     *
     ...
     * blockchain and a received blockchain.
     * It verifies the validity of the received blockchain and compares it with the
     * current blockchain to decide
     * which one to keep based on various criteria such as creation time, mining
     * points, and transaction ledgers.
     * <br>
    ...
    /**
     * this method shares any transaction that might be missing between
     * the local and the received blockchain from their last blocks between them.
     * it ensures that both blockchains have the same set of transactions in their
     * last blocks by adding any missing transactions from one blockchain to the
     * other.
     * 
    ...
    /**
     * Check each blockchain’s recorded mining points.
     * In case of a tie, we will determine the outcome by
     * their recorded luck value, which is just a large random number.
     * 
    ...
    /**
     * determine if both blockchains have the same initial block.
     * If they don’t, we will use the one that got created first.
     * 
     * @param receivedBC The received blockchain to compare with the current
     * @return The updated current blockchain if it wins, or null if the received
     */
    ...
    /**
     * determines if any method is outdated and discards it.
     * If the received one
     * is outdated, then we keep using ours.
     * If ours is outdated, we discard ours
     * and use the received one from this point forward.
     * If both are outdated, then we do nothing regarding consensus, and we wait
     * to receive an up-­to-­date blockchain.
     * If both are up-to-date, then we move on with the
     * consensus checks.
     * 
     * @param receivedBC The received blockchain to be compared with the current
     * @return The updated current blockchain if it wins, or null if the received
